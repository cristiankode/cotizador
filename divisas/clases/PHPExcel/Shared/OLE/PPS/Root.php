...
class PHPExcel_Shared_OLE_PPS_Root extends PHPExcel_Shared_OLE_PPS {
    ...
    public function save($filename) {
        ...
        $this->_BIG_BLOCK_SIZE = pow(
                2, (isset($this->_BIG_BLOCK_SIZE)) ? self::adjust2($this->_BIG_BLOCK_SIZE) : 9
        );
        $this->_SMALL_BLOCK_SIZE = pow(
                2, (isset($this->_SMALL_BLOCK_SIZE)) ? self::adjust2($this->_SMALL_BLOCK_SIZE) : 6
        );
    ...
    public function _saveHeader($iSBDcnt, $iBBcnt, $iPPScnt) {
        ...
        // Save Header
        fwrite(
                $FILE, "\xD0\xCF\x11\xE0\xA1\xB1\x1A\xE1"
                . "\x00\x00\x00\x00"
                . "\x00\x00\x00\x00"
                . "\x00\x00\x00\x00"
                . "\x00\x00\x00\x00"
        ...
        if ($iBdCnt < $i1stBdL) {
            fwrite(
                    $FILE, pack("V", -2) // Extra BDList Start
